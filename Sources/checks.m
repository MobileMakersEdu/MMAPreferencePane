...
Promise *MMACheckXcode(NSNumber* xcodeBaseBundleId, NSNumber* currentXcodeVersion) {
    return mdfind(@"Xcode").then(^(NSString *unused, NSArray *paths){
        ...
        if (versions.none(^(id version){return [version intValue] >= xcodeBaseBundleId.intValue;}))
            @throw @NO;
    }).catch(^{
        id info = @{
            NSLocalizedDescriptionKey:[NSString stringWithFormat:@"You need to install Xcode %@", currentXcodeVersion],
...
//    });
//
//    id p2 = [NSTask:@"/usr/bin/git config --global user.name"].promise.then(^(NSString *stdout){
//        if (!stdout.chuzzle)
//            @throw @1;
//    }).catch(^{
//        id info = @{
...
//    });
//
//    id p3 = [NSTask:@"/usr/bin/git config --global user.email"].promise.then(^(NSString *stdout){
//        if (!stdout.chuzzle)
//            @throw @1;
//    }).catch(^{
//        id info = @{
